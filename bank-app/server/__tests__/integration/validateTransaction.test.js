...
beforeEach(async () => {
    const mockLoyaltyProgram = new LoyaltyPrograms(mockLoyaltyProgramData);
    await mockLoyaltyProgram.save();
})

afterEach(async () => {
    await LoyaltyPrograms.findOneAndDelete({ programId: loyaltyProgramId });
})
...
describe('validateTransaction middleware function unit tests', () => {
  ...
  test('validateTransaction returns status code 400 if loyaltyProgramId does not exist', async () => {
    ...
    expect(response.data.error).toEqual("Loyalty program not found.")
