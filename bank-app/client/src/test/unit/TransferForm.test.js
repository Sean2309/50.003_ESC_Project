  ...
  ;
    ...
    it('point validation: fuzzing transfer amount input, allowing invalid inputs such as alphabets', async () => {
      ...
      await act(async () => {
        ...
        await waitFor(() => {
            if (regex.test('123456789S') == false || int_regex.test(fuzzTransferAmountInput) == false || parseInt(fuzzTransferAmountInput)>mockedUserProfile.abcPoints) {
              expect(axios.post).not.toHaveBeenCalled();
            }
            else if (regex.test('123456789S') == true && int_regex.test(fuzzTransferAmountInput) == true && parseInt(fuzzTransferAmountInput)<=mockedUserProfile.abcPoints) {
              expect(axios.post).toHaveBeenCalled();
            }
          });
    });
    ...
    it('point validation: fuzzing transfer amount input, only integer values', async () => {
      ...
      await act(async () => {
        ...
        await waitFor(() => {
            if (regex.test('123456789S') == false || parseInt(fuzzTransferAmountInput)>mockedUserProfile.abcPoints) {
              expect(axios.post).not.toHaveBeenCalled();
            }
            else if (regex.test('123456789S') == true && parseInt(fuzzTransferAmountInput)<=mockedUserProfile.abcPoints) {
              expect(axios.post).toHaveBeenCalled();
            }
          });
    });
